...
app.post('/api/meetings', (req, res) => {
  const id = (req.body && req.body.id) || genId(8);
  const m = ensureMeeting(id);
  const origin = req.headers.origin || ALLOWED_ORIGINS[0] || 'http://localhost:5173';
...
app.post('/api/meetings/:id/lock', (req,res) => {
  ...
  res.json({ ok:true, locked: m.locked });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('join_meeting', ({ meetingId, name, role, pin }, ack) => {
    ...
    if (role === 'moderator') {
      if (pin !== m.pin) return ack?.({ ok:false, error:'PIN non valido' });
      socket.data.role = 'moderator';
    }
    socket.data.meetingId = meetingId;
    socket.data.name = (name || '').trim();
    socket.join(meetingId);
    ack?.({ ok:true, state: stateForClient(m), pin: role==='moderator' ? m.pin : undefined });
  });

  socket.on('enqueue', ({ topic, name, requestedDurationSec }, ack) => {
    const mid = socket.data.meetingId;
    if (!mid) return ack?.({ ok:false, error:'Non in meeting' });
    const m = ensureMeeting(mid);
    if (m.locked) return ack?.({ ok:false, error:'Coda bloccata' });
    ...
    const item = {
      ...
      topic: (topic || '').trim(),
      requestedDurationSec: safeReqDur,
      status: 'waiting',
      priority: 0,
      createdAt: Date.now()
    };
  ...
  socket.on('start_speaking', ({ id, durationSec }, ack) => {
    const mid = socket.data.meetingId;
    const m = ensureMeeting(mid);
    if (socket.data.role !== 'moderator') return ack?.({ ok:false, error:'Solo moderatore' });

    const idx = m.queue.findIndex(q => q.id === id);
    if (idx < 0) return ack?.({ ok:false, error:'Elemento non trovato' });
  ...
  socket.on('promote', ({ id }, ack) => {
    const mid = socket.data.meetingId;
    const m = ensureMeeting(mid);
    if (socket.data.role !== 'moderator') return ack?.({ ok:false });
